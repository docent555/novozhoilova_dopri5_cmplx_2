      SUBROUTINE DOPRI5_F(N,FCN,X,Y,XEND,
     &                  RTOL,ATOL,ITOL,
     &                  SOLOUT,IOUT,
...
C -------- CALL TO CORE INTEGRATOR ------------
      CALL DOPCOR_F(N,FCN,X,Y,XEND,HMAX,H,RTOL,ATOL,ITOL,IPRINT,
...
C
      SUBROUTINE DOPCOR_F(N,FCN,X,Y,XEND,HMAX,H,RTOL,ATOL,ITOL,IPRINT,
     &   SOLOUT,IOUT,IDID,NMAX,UROUND,METH,NSTIFF,SAFE,BETA,FAC1,FAC2,
...
C ---------------------------------------------------------- 
      ...
      LOGICAL REJECT,LAST 
      EXTERNAL FCN
      COMMON /CONDO5_F/XOLD,HOUT
C *** *** *** *** *** *** ***
C  INITIALISATIONS
C *** *** *** *** *** *** *** 
      IF (METH.EQ.1) CALL CDOPRI_F(C2,C3,C4,C5,E1,E3,E4,E5,E6,E7,
...
C --- INITIAL PREPARATIONS   
      ...
      HMAX=ABS(HMAX)     
      IORD=5  
      IF (H.EQ.0.D0) H=HINIT_F(N,FCN,X,Y,XEND,POSNEG,K1,K2,K3,IORD,
...
C --- FAIL EXIT
  ...
  79  CONTINUE
      IF (IPRINT.GT.0) WRITE(IPRINT,979)X
 979  FORMAT(' EXIT OF DOPRI5_F AT X=',E18.4) 
      IDID=2
      RETURN
      END
C
      FUNCTION HINIT_F(N,FCN,X,Y,XEND,POSNEG,F0,F1,Y1,IORD,
     &                 HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
C ----------------------------------------------------------
...
C ----- COMPUTE PLACE OF II-TH COMPONENT 
   ...
   5  CONTINUE
      ...
      THETA=(X-XOLD)/H
      THETA1=1.D0-THETA
      CONTD5_F=CON(I)+THETA*(CON(ND+I)+THETA1*(CON(2*ND+I)+THETA*
     &           (CON(3*ND+I)+THETA1*CON(4*ND+I))))
      RETURN
      END
C
      SUBROUTINE CDOPRI_F(C2,C3,C4,C5,E1,E3,E4,E5,E6,E7,
     &                    A21,A31,A32,A41,A42,A43,A51,A52,A53,A54,
     &                    A61,A62,A63,A64,A65,A71,A73,A74,A75,A76,
     &                    D1,D3,D4,D5,D6,D7)
C ----------------------------------------------------------
